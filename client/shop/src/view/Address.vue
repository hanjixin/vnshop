      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="item in list " :class="{'addCheck':item.isDefault}" @click="setDefault(item.addressId)">
                  <dl>
                    <dt>{{item.userName}}</dt>
                  ...
                  <div class="addr-opration addr-default" v-if="!item.isDefault">点击设为默认地址</div>
                </li>
                <li class="addr-new">
          ...
          <div class="next-btn-wrap">
            <router-link :to="{'name':'order',query:{'addressId':addressId}}" class="btn btn--m btn--red">下一步</router-link>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
   ...
   data:function () {
    ...
    methods:{
      getAddress:function () {
        this.$http.get('/goods/lookAddress').then(res=>{
      ...
      setDefault:function (addressId) {
        this.$http.post('/users/setDefault',{addressId:addressId}).then(res=>{
      ...
      delAddress:function (addressId) {
        this.$http.post('/users/delAddress',{addressId:addressId}).then(res=>{
          console.log(res.data)
          this.getAddress()
        })
      }
